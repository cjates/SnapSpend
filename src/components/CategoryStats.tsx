...
export default function CategoryStats({ receipts, budgets, onUpdateBudget }: CategoryStatsProps) {
  ...
  return (
    <div className="space-y-6 animate-fade-in text-slate-800" id="stats-summary-panel">
      {/* Dynamic Session Dashboard Info Block - Navy Blue styling */}
      <div className="bg-[#0f172a] rounded-xl border border-slate-800 shadow-lg p-5 flex flex-col gap-5 text-white animate-fade-in" id="summary-category-report">
        <div>
        ...
        <div className="grid grid-cols-2 gap-4" id="aggregations-counter-grid">
          <div className="p-3 bg-slate-800/80 border border-slate-700/85 rounded-lg">
            <p className="text-[9px] text-slate-400 uppercase font-bold mb-0.5">
              Total Spend
            </p>
            <p className="text-xl font-bold text-white tracking-tight font-sans">
              ${totalSpend.toFixed(2)}
            </p>
            <p className="text-[9px] text-slate-405 mt-1 font-mono">
        ...
        <div className="relative pt-1">
          <div className="flex mb-1 items-center justify-between text-[11px]">
            <span className="text-slate-300 font-medium font-sans">Overall Remaining:</span>
            <span className={`font-mono font-bold ${isOverallOverBudget ? "text-rose-400 animate-pulse" : "text-emerald-400"}`}>
          ...
          <div className="overflow-hidden h-2 text-xs flex rounded-full bg-slate-950 border border-slate-800">
            <div
              ...
              className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-500 ${
                isOverallOverBudget ? "bg-rose-500" : "bg-emerald-500"
              }`}
            />
          </div>
      ...
      {/* Category Budgets Status Overview Board - Gray card styling */}
      <div className="bg-slate-100/90 rounded-xl border border-slate-200 p-5 space-y-5 shadow-sm text-slate-850" id="category-budgets-section">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 border border-slate-300 text-slate-750 hover:text-slate-900 hover:bg-slate-200 bg-white rounded-md text-[10px] font-semibold transition cursor-pointer shadow-sm"
        ...
        {/* Collapsible Budget Configuration Panel - inside a nested gray frame */}
        {showBudgetEditor && (
          <div className="p-3.5 bg-slate-200/70 border border-slate-300 rounded-lg space-y-3.5 animate-slide-up" id="budgets-editor-form">
              ...
              {(Object.keys(CATEGORY_META) as ExpenseCategory[]).map((cat) => (
                <div key={cat} className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="px-2 py-1.5 bg-white border border-slate-300 text-slate-800 rounded text-xs outline-none focus:border-slate-500 font-mono tracking-tight"
                  />
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-3 gap-2" id="budget-status-summary-row">
          <div className="p-2 border border-slate-200 bg-slate-200/40 rounded text-center shadow-xs">
            <span className="text-[18px] font-bold text-rose-600 block font-mono">
              {overBudgetCount}
            </span>
          ...
          <div className="p-2 border border-slate-200 bg-slate-200/40 rounded text-center shadow-xs">
            <span className="text-[18px] font-bold text-amber-600 block font-mono">
              {warningBudgetCount}
            </span>
          ...
          <div className="p-2 border border-slate-200 bg-slate-200/40 rounded text-center shadow-xs">
            <span className="text-[18px] font-bold text-emerald-600 block font-mono">
              {optimalBudgetCount}
            </span>
        ...
        <div className="space-y-4" id="category-budgets-list">
          {budgetStatuses.map(({ category, spent, budget, remaining, percentUsed, isOver, isWarning, statusColor, progressColor, meta }) => {
            return (
              <div 
                key={category} 
                className="space-y-1.5 p-3.5 bg-white hover:bg-slate-50 border border-slate-200/80 rounded-lg transition shadow-xs"
                id={`budget-bar-row-${category}`}
              >
                <div className="flex justify-between items-center text-xs">
                  <div className="flex items-center gap-2">
                    <div className={`w-2.5 h-2.5 rounded-full ${meta.color}`}></div>
                    <span className="font-bold text-slate-800 capitalize font-sans">{meta.label}</span>
                  </div>
                ...
                <div className="flex justify-between items-center pt-0.5">
                  ...
                  {isOver ? (
                    <span className="text-[9px] font-bold text-rose-600 px-1.5 py-0.25 bg-rose-50 border border-rose-100 rounded uppercase">
